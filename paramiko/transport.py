...
from cryptography.hazmat.primitives.ciphers.algorithms import AES, ARC4, Blowfish, TripleDES
...
class Transport (threading.Thread):
    ...
    _cipher_info = {
        'aes128-ctr': {'class': AES, 'mode': CTR, 'block-size': 16, 'key-size': 16},
        'aes256-ctr': {'class': AES, 'mode': CTR, 'block-size': 16, 'key-size': 32},
        'blowfish-cbc': {'class': Blowfish, 'mode': CBC, 'block-size': 8, 'key-size': 16},
        'aes128-cbc': {'class': AES, 'mode': CBC, 'block-size': 16, 'key-size': 16},
        'aes256-cbc': {'class': AES, 'mode': CBC, 'block-size': 16, 'key-size': 32},
        '3des-cbc': {'class': TripleDES, 'mode': CBC, 'block-size': 8, 'key-size': 24},
    ...
    def _get_cipher(self, name, key, iv, encrypt):
        if name not in self._cipher_info:
            raise SSHException('Unknown client cipher ' + name)
        if name in ('arcfour128', 'arcfour256'):
            # arcfour cipher
            cipher = Cipher(self._cipher_info[name]['class'](key), None, default_backend())
            if encrypt == self._ENCRYPT:
            ...
            cipher = Cipher(self._cipher_info[name]['class'](key), self._cipher_info[name]['mode'](iv), default_backend())
            if encrypt == self._ENCRYPT:
                engine = cipher.encryptor()
            else:
                engine = cipher.decryptor()
